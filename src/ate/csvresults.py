...
class Csv:
    ...
    class TestDataRecord(Record):
        ...
        class Test(Record.Test):
            ...
            def __init__(self, outer:Csv.TestDataRecord, name:str, data) -> None:
                if not isinstance(outer, Csv.TestDataRecord):
                    raise TypeError(f"exp type: {Csv.TestDataRecord}, recv type {type(outer)}")
                self._name = name
                self.data = data
                self.operator = None
                self.record = outer
